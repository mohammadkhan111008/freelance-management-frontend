...
export default function Sidebar({ currentPage, setCurrentPage, isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) {
  ...
  return (
    <>
      {/* Mobile Backdrop Overlay: Dims the background when the sidebar drawer is slid open on phone viewports */}
      {isMobileOpen && (
        <div 
      ...
      <aside 
        className={`bg-white border-r border-slate-200 fixed inset-y-0 left-0 z-50 flex flex-col justify-between shadow-sm transition-all duration-300 ease-in-out
          /* Mobile Drawer Layer logic */
          ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} 
          /* Desktop Sidebar logic overrides */
      ...
      >
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className={`flex items-center justify-between px-2 py-1 mb-8 ${isCollapsed ? 'md:justify-center' : ''}`}>
            ...
            {/* Mobile Close Button: only visible on responsive small screens */}
            <button 
              onClick={() => setIsMobileOpen(false)}
        ...
        {/* Footer Panel & Desktop Collapse Button Toggle */}
        <div className="border-t border-slate-100 bg-slate-50/60 p-3 space-y-2">
          ...
          {/* Desktop-only Expand Toggle Action */}
          <button
